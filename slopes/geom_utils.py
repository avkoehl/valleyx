...
def extend_linestring(linestring, point):
    ...
    distances = gpd.GeoSeries([Point(linestring.coords[0]), Point(linestring.coords[-1])]).distance(point)
...
def add_point_to_polygon_exterior(polygon, point):
    ...
    _ , p2 = nearest_points(point, exterior_coords_mp) # get nearest boundary point to Point
